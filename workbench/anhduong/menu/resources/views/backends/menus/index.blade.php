        ...
        <div class="row">
            <div class="col-xs-12">
                ...
                <div class="box">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="sbox-tools pull-right">
                                        <a data-original-title="Clear Search" href="{!! route('admin.menu.index') !!}" class="btn btn-xs btn-white tips" title=""><i class="fa fa-trash-o"></i> Clear Search </a>
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                    <div id="example1_length" class="dataTables_length">
                                        {!! Form::open(array('route' => 'admin.menu.index','role' => 'form', 'method'=> 'get')) !!}
                                ...
                                <div class="col-sm-6">
                                    <div class="dataTables_filter pull-right" id="example1_filter">
                                        ...
                                        {!! Form::select('searchFields', array( 'id;title;slug' => 'Search by All', 'id' => 'Search by ID', 'title' =>
                                        'Search by Title', 'slug' => 'Search by Slug'),
                    ...
                    <!-- /end .box-body -->
                    {!! Form::open(array('route' => array('admin.menu.updatemulti'), 'method' => 'post')) !!}
                    <!-- .box-body -->
                        ...
                        @if(isset($items) && $items != null)
                            <table class="table table-hover">
                                ...
                                <tr>
                                    ...
                                    {!! Backend::columns_sort('Title', 'title', $sortedBy, $options) !!}
                                    {!! Backend::columns_sort('Slug', 'slug', $sortedBy, $options) !!}
                                ...
                                @foreach ($items as $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->title }}</td>
                                        <td>{!! Backend::showLink($options, route('admin.menu.show', $item->id), $item->slug) !!}
                                        </td>
                                        ...
                                        <td>
                                            {!! Backend::showButton($options, route('admin.menu.edit', $item->id), $item->id) !!}
                                        </td>
                                    </tr>
                                @endforeach
                            </table>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title">{{ trans('messages.delete', ['name' => trans('backend.menu') ])}}</h4>
          </div>
          <div class="modal-body">
            <p>{{ trans('messages.delete_message', ['name' => trans('backend.menu') ]) }}</p>
          </div>
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-outline action_confirm" data-method="delete" onclick="if ($(this).hasClass('action_confirm')) { $(this).find('form').submit(); }">{{ trans('messages.yes') }}
                {!! Form::open(array('route' => array('admin.menu.destroy', 1), 'method' => 'delete')) !!}
                {!! Form::close() !!}
            </button>
          </div>
        </div>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $('.confirm-delete').on('click', function(e) {
                ...
                $('#myModal').find('form').attr('action', '{{ config('app.url') }}'+'/admin/menu/'+ id);
            });
        ...
        function submitSearch()
        {
            ...
            var link  = "{{ route('admin.menu.index',array('searchFields' => 'searchFields', 'search' => 'search')) }}";
