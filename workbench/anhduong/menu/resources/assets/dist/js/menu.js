...
var MenuNestable = function() {
    ...
    return {
        ...
        handleNestableMenu: function() {
            ...
            $body.on('click', '.widget-menu .widget-title', function(e) {
                e.preventDefault();
                var widget = $(this).closest('.widget').children('.widget-body');
                if ($(this).hasClass('opened')) {
                    $(this).removeClass('opened').addClass('closed');
                    widget.slideUp(200);
                } else {
                    $('.slimScrollDiv').closest('.widget').find('.widget-body').slideUp(200);
                    $('.slimScrollDiv').closest('.widget').find('.widget-title').removeClass('opened').addClass('closed');
                    $('.slimScrollDiv').closest('.widget').find('.tools a').removeClass('collapse').addClass('expand');
                    $(this).removeClass('closed').addClass('opened');
                    widget.slideDown(200);
                }
                $(this).find('.tools a').toggleClass('expand').toggleClass('collapse');
                $(this).closest('.widget').removeClass('widget-hide');
            });
        }
    };
}();
