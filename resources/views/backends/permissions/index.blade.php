            ...
            <div class="col-xs-12">
                ...
                <div class="box">
                    <div class="box-header">
                        <h3 class="box-title"><i class="fa fa-list" aria-hidden="true"></i> {{ trans('messages.list', ['name' => trans('backend.permission')]) }}</h3>

                        <div class="pull-right">
                            <a href="{!! route('admin.permission.export', array('type' => 'csv')) !!}"><button id="button-shipping" class="btn btn-info" title="Export to CSV"><i class="fa fa-file-text"></i></button></a>
                            <a href="{!! route('admin.permission.export', array('type' => 'pdf')) !!}"><button id="button-invoice" class="btn btn-info" title="Export to Pdf"><i class="fa fa-print"></i></a>
                            </button>
                            <a title="Create User" href="{{ route('admin.permission.create') }}"
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="sbox-tools pull-right">
                                        <a data-original-title="Clear Search" href="{!! route('admin.permission.index') !!}" class="btn btn-xs btn-white tips" title=""><i class="fa fa-trash-o"></i> Clear Search </a>
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                    <div id="example1_length" class="dataTables_length">
                                        {!! Form::open(array('route' => 'admin.permission.index','role' => 'form', 'method'=> 'get')) !!}
                                ...
                                <div class="col-sm-6">
                                    <div class="dataTables_filter pull-right" id="example1_filter">
                                        ...
                                        {!! Form::select('searchFields', array( 'id;area;permission;actions' => 'Search by All', 'id' => 'Search by ID', 'area' =>
                                        'Search by Area', 'permission' => 'Search by Permission', 'actions' => 'Search by Actions'),
                    ...
                    <!-- /end .box-body -->
                    {!! Form::open(array('route' => array('admin.permission.updatemulti'), 'method' => 'post')) !!}
                    <!-- .box-body -->
                        ...
                        @if(isset($items) && $items != null)
                            <table class="table table-hover">
                                ...
                                <tr>
                                    ...
                                    {!! Backend::columns_none_sort('Area', 'area', $sortedBy, $options) !!}
                                    {!! Backend::columns_none_sort('Permission', 'permission', $sortedBy, $options) !!}
                                    {{-- {!! Backend::columns_none_sort('Actions', 'actions', $sortedBy, $options) !!} --}}
                                ...
                                @foreach ($items as $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->area }}</td>
                                        <td>{!! Backend::showLink($options, route('admin.permission.show', $item->id), $item->permission) !!}
                                        </td>
                                        {{-- <td>{{ $item->actions }}</td> --}}
                                        ...
                                        <td>
                                            {!! Backend::showButton($options, route('admin.permission.edit', $item->id), $item->id) !!}
                                        </td>
                                    </tr>
                                @endforeach
                            </table>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title">{{ trans('messages.delete', ['name' => trans('backend.permission') ])}}</h4>
          </div>
          <div class="modal-body">
            <p>{{ trans('messages.delete_destroy', ['name' => trans('backend.permission') ]) }}</p>
          </div>
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-outline action_confirm" data-method="delete" onclick="if ($(this).hasClass('action_confirm')) { $(this).find('form').submit(); }">{{ trans('messages.yes') }}
                {!! Form::open(array('route' => array('admin.permission.destroy', 1), 'method' => 'delete')) !!}
                {!! Form::close() !!}
            </button>
          </div>
        </div>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $('.confirm-delete').on('click', function(e) {
                ...
                $('#myModal').find('form').attr('action','http://laravel53.dev/admin/permission/'+ id);
            });
        ...
        function submitSearch()
        {
            ...
            var link  = "{{ route('admin.permission.index',array('searchFields' => 'searchFields', 'search' => 'search')) }}";
