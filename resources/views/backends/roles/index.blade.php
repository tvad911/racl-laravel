                ...
                <div class="box">
                    <div class="box-header">
                        ...
                        <div class="pull-right">
                            <a href="{!! route('admin.role.export', array('type' => 'csv')) !!}"><button id="button-shipping" class="btn btn-info" title="Export to CSV"><i class="fa fa-file-text"></i></button></a>
                            <a href="{!! route('admin.role.export', array('type' => 'pdf')) !!}"><button id="button-invoice" class="btn btn-info" title="Export to Pdf"><i class="fa fa-print"></i></a>
                            </button>
                            <a title="Create User" href="{{ route('admin.role.create') }}"
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="sbox-tools pull-right">
                                        <a data-original-title="Clear Search" href="{!! route('admin.role.index') !!}" class="btn btn-xs btn-white tips" title=""><i class="fa fa-trash-o"></i> Clear Search </a>
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                    <div id="example1_length" class="dataTables_length">
                                        {!! Form::open(array('route' => 'admin.role.index','role' => 'form', 'method'=> 'get')) !!}
                    ...
                    <!-- /end .box-body -->
                    {!! Form::open(array('route' => array('admin.role.updatemulti'), 'method' => 'post')) !!}
                    <!-- .box-body -->
                        ...
                        @if(isset($items) && $items != null)
                            <table class="table table-hover">
                                ...
                                <tr>
                                    ...
                                    {!! Backend::columns_none_sort('Filter', 'filter', $sortedBy, $options) !!}
                                ...
                                @foreach ($items as $item)
                                    <tr>
                                        ...
                                        <td>{!! Backend::showLink($options, route('admin.role.show', $item->id), $item->name) !!}</td>
                                        <td>{{ $item->filter }}</td>
                                        <td>{{ $item->created_at }}</td>
                                        <td>{{ $item->updated_at }}</td>
                                        <td>{{ $item->created_by }}</td>
                                        <td>
                                            {!! Backend::showButton($options, route('admin.role.edit', $item->id), $item->id) !!}
                                        </td>
                                    </tr>
                                @endforeach
                            </table>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title">{{ trans('messages.delete', ['name' => trans('backend.role') ])}}</h4>
          </div>
          <div class="modal-body">
            <p>{{ trans('messages.delete_destroy', ['name' => trans('backend.role') ]) }}</p>
          </div>
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-outline action_confirm" data-method="delete" onclick="if ($(this).hasClass('action_confirm')) { $(this).find('form').submit(); }">{{ trans('messages.yes') }}
                {!! Form::open(array('route' => array('admin.role.destroy', 1), 'method' => 'delete')) !!}
                {!! Form::close() !!}
            </button>
          </div>
        </div>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $('.confirm-delete').on('click', function(e) {
                ...
                $('#myModal').find('form').attr('action','http://laravel53.dev/admin/role/'+ id);
            });
        ...
        function submitSearch()
        {
            ...
            var link  = "{{ route('admin.role.index',array('searchFields' => 'searchFields', 'search' => 'search')) }}";
