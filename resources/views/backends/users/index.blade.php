            ...
            <div class="col-xs-12">
                ...
                <div class="box">
                    <div class="box-header">
                        <h3 class="box-title"><i class="fa fa-list" aria-hidden="true"></i> {{ trans('messages.list', ['name' => trans('backend.user')]) }}</h3>

                        <div class="pull-right">
                            <a href="{!! route('admin.user.export', array('type' => 'csv')) !!}"><button id="button-shipping" class="btn btn-info" title="Export to CSV"><i class="fa fa-file-text"></i></button></a>
                            <a href="{!! route('admin.user.export', array('type' => 'pdf')) !!}"><button id="button-invoice" class="btn btn-info" title="Export to Pdf"><i class="fa fa-print"></i></a>
                            </button>
                            <a title="Create User" href="{{ route('admin.user.create') }}"
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="sbox-tools pull-right">
                                        <a data-original-title="Clear Search" href="{!! route('admin.user.index') !!}" class="btn btn-xs btn-white tips" title=""><i class="fa fa-trash-o"></i> Clear Search </a>
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                    <div id="example1_length" class="dataTables_length">
                                        {!! Form::open(array('route' => 'admin.user.index','role' => 'form', 'method'=> 'get')) !!}
                                ...
                                <div class="col-sm-6">
                                    <div class="dataTables_filter pull-right" id="example1_filter">
                                        ...
                                        {!! Form::select('searchFields', array( 'id;username;email' => 'Search by All', 'id' => 'Search by ID', 'username' =>
                                        'Search by Username', 'email' => 'Search by Email'),
                    ...
                    <!-- /end .box-body -->
                    {!! Form::open(array('route' => array('admin.user.updatemulti'), 'method' => 'post')) !!}
                    <!-- .box-body -->
                        ...
                        @if(isset($items) && $items != null)
                            <table class="table table-hover">
                                ...
                                <tr>
                                    ...
                                    {!! Backend::columns_sort('#ID', 'id', $sortedBy, $options) !!}
                                    {!! Backend::columns_sort('Username', 'username', $sortedBy, $options) !!}
                                ...
                                @foreach ($items as $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->username }}</td>
                                        <td>{!! Backend::showLink($options, route('admin.user.show', $item->id), $item->email) !!}
                                        </td>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title">{{ trans('messages.delete', ['name' => trans('backend.user') ])}}</h4>
          </div>
          <div class="modal-body">
            <p>{{ trans('messages.delete_message', ['name' => trans('backend.user') ]) }}</p>
          </div>
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-outline action_confirm" data-method="delete" onclick="if ($(this).hasClass('action_confirm')) { $(this).find('form').submit(); }">{{ trans('messages.yes') }}
                {!! Form::open(array('route' => array('admin.user.destroy', 1), 'method' => 'delete')) !!}
                {!! Form::close() !!}
            </button>
          </div>
        </div>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $('.confirm-delete').on('click', function(e) {
                ...
                $('#myModal').find('form').attr('action','http://laravel53.dev/admin/user/'+ id);
            });
        ...
        function submitSearch()
        {
            ...
            var link  = "{{ route('admin.user.index',array('searchFields' => 'searchFields', 'search' => 'search')) }}";
